...
class ProgramHydrationController:
    ...
    def build_inspiration_index(
        ...
        uses_by_child = self.inspirations.list_for_children(child_program_ids)
    ...
    def record_to_program(
        ...
        *,
        ...
        if evaluation_index is None:
            evaluation_index = self.build_evaluation_index([record.id])
        if proposal_index is None:
            proposal_index = self.build_proposal_index([record.id])
        if embedding_index is None:
            embedding_index = self.build_embedding_index([record.id])
        if projection_index is None:
            projection_index = self.build_projection_index([record.id])
        inspiration_lists = (
            inspiration_index.get(record.id, {})
            if inspiration_index is not None
            else self.build_inspiration_index([record.id]).get(record.id, {})
        )
