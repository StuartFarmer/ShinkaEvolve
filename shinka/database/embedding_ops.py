...
def list_by_island(session: Session, island_idx: int) -> List[tuple[str, list[float]]]:
...
def list_all(session: Session) -> List[tuple[str, list[float]]]:
    rows = session.execute(
        select(
            ProgramEmbeddingRecord.program_id,
            ProgramEmbeddingRecord.vector_json,
...
def update_features(
    ...
    *,
    ...
    embedding_record = session.scalar(
        select(ProgramEmbeddingRecord).where(ProgramEmbeddingRecord.program_id == program_id)
    )
    ...
    projection_2d.cluster_id = embedding_cluster_id
    projection_3d.coords_json = _clean_nan_values(embedding_pca_3d)
    projection_3d.cluster_id = embedding_cluster_id


def recompute(
    session: Session,
    *,
    embedding_client_factory: Callable[[], Optional[EmbeddingClient]] | None,
    read_only: bool,
    num_clusters: int = 4,
) -> None:
    if read_only:
